Perform a modification of your Next.js project.

We're going to modify some Next.js project code.
...
- [ ] Run `npm test` and make sure **all tests pass**. A non-zero exit is a hard blocker — do not proceed to the next phase until the suite is green.
- [ ] Run `prettier --write .` to make sure that the formatting is correct.
...
- [ ] After committing the change, if the Next.js dev server is running (`next dev`), the app will hot-reload automatically — verify the changes look correct in the browser.

In the last phase of the plan, include steps to:

- [ ] Run `npm run test:coverage` and record the coverage summary in the Journal section of MODIFICATION_IMPLEMENTATION.md.
- [ ] Update any README.md file for the package with relevant information from the modification (if any).
- [ ] Update the CLAUDE.md file in the project directory so that it still correctly describes the app, its purpose, and implementation details and the layout of the files.
