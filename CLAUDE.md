...
├── components/
│   ├── MapWithNav.tsx          # 'use client' state wrapper — owns selectedAreaId, layerVisibility, infoPanelData; composes AreaNav + MapView + LayerPanel + InfoPanel
...
│   └── milsymbol.test.ts   # createMilsymbolImage — data URL format, option passthrough, onerror rejection
    └── components/
        ...
        ├── MapWithNav.test.tsx # state wiring tests (stubbed AreaNav + MapView + LayerPanel + InfoPanel)
...
- **Mock strategy**:
  ...
  - Child components (`AreaNav`, `MapView`, `LayerPanel`, `InfoPanel`) — `vi.mock` with data-attribute stubs in `MapWithNav` tests to isolate state wiring.
...
- **Coverage summary (last run — 166 tests)**: `areas.ts` 100%, `db.ts` 100%, `layers.ts` 100%, `milsymbol.ts` 100%, all API routes 100%, `AreaNav.tsx` 100%, `LayerPanel.tsx` 100%, `InfoPanel.tsx` 100%, `MapWithNav.tsx` 100%, `MapView.tsx` ~94% stmts/lines (branch gaps at async null guards inside callbacks — expected), `MapLoader.tsx` 100% stmts/lines.
