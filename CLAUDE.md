...
│       ├── route-intelligence/
│       │   └── route.ts        # POST /api/route-intelligence → RouteIntelligence JSON; PostGIS roads query: ST_DWithin 22 m candidate set + 10 m corridor overlap filter (ST_Intersection > 15 m) to suppress side-road false positives + bridges ST_DWithin 50 m; classifies hazards against vehicle profile
│       └── custom-layers/
...
│   │                           #   "Plan a Route" CTA button (top-4 right-4, blue); unified weather panel (top-left one-box);
│   │                           #   custom layer props passed to LayerPanel (no standalone CustomLayerPanel overlay)
│   ├── RoutePanel.tsx          # 'use client' forwardRef — right-side collapsible route planning panel (right-4 bottom-10 w-80);
...
│   │                           #   collapse/expand chevron in header; close button removes panel from DOM
│   ├── WeatherWidget.tsx       # 'use client' — fetches /api/weather, shows avg temp ± spread + rain stats; bare prop strips outer panel div
│   ├── DatePicker.tsx          # 'use client' — two <select> dropdowns (month/day) with leap-year day clamping; bare prop strips outer panel div
│   ├── AreaNav.tsx             # 'use client' — top-centered AOI navigation button strip
│   ├── LayerPanel.tsx          # 'use client' — floating bottom-left toggle panel (w-56); sections: Basemap/Terrain/Elevation/
│   │                           #   Vegetation/Comms/Infrastructure + optional Custom Layers section via customLayerProps prop;
│   │                           #   renders CustomLayerSection inline when customLayerProps is provided
│   ├── CustomLayerPanel.tsx    # 'use client' — exports CustomLayerSection (inner body, no wrapper) + legacy CustomLayerPanel default
│   │                           #   export (standalone panel wrapper, kept for test compatibility); CustomLayerSection is rendered
│   │                           #   inside LayerPanel as a section; standalone CustomLayerPanel overlay is no longer used in MapWithNav
...
│   │   ├── route-intelligence.test.ts  # POST /api/route-intelligence — validation, road/bridge hazard rules, Infantry mass-skip, sorting, 503/500
    ...
    └── components/
        ...
        ├── MapWithNav.test.tsx # state wiring tests (stubbed AreaNav + MapView + LayerPanel (with embedded custom-layer stub) + InfoPanel + WeatherWidget + DatePicker + RoutePanel); intelligence + focusedHazard wiring
...
- **Coverage summary (last run — 450 tests)**: All files 92.77% stmts / 85.74% branch / 85.38% functions. All API routes 100% except `route-intelligence/route.ts` ~96%. All lib modules 100%. `InfoPanel.tsx` 100%, `DatePicker.tsx` 100%, `WeatherWidget.tsx` ~98.5%, `LayerPanel.tsx` 100%, `DrawingToolbar.tsx` 100%, `FeatureDialog.tsx` ~98%, `RoutePanel.tsx` ~98.6%, `CustomLayerPanel.tsx` ~98.4%, `MapWithNav.tsx` ~92%, `MapView.tsx` ~86% (branch gaps at async null guards — expected).
...
- **Weather widget**: When `selectedAreaId !== null`, `MapWithNav` renders a unified weather panel (`absolute left-4 top-16 z-10 w-52 rounded-lg border ...`). The panel contains a date header row (label + `DatePicker bare`) and `WeatherWidget bare` below it — a single visual box. Both `WeatherWidget` and `DatePicker` accept a `bare?: boolean` prop; when true they omit their outer panel div so the parent panel provides the wrapper. `WeatherWidget` fetches `/api/weather?region&month&day` on prop change; shows avg temp ± spread, ↑max/↓min, rain probability %, avg mm on wet days. Falls back to "No data" when `sampleSize === 0`. `DatePicker` renders two `<select>` dropdowns (Month Jan–Dec, Day 1–31 clamped to month max); Feb max is 29.
...
- **LayerPanel**: Floating dark-slate panel (`absolute left-4 bottom-10 w-56`) with collapsible sections — BASEMAP, TERRAIN, ELEVATION, VEGETATION, COMMS, INFRASTRUCTURE, and optionally CUSTOM LAYERS. Accepts `customLayerProps?: CustomLayerPanelProps`; when provided renders `<CustomLayerSection>` (from `CustomLayerPanel.tsx`) as the bottom section. `MapWithNav` passes custom layer props here directly; there is no separate `CustomLayerPanel` overlay. Each row is a checkbox with a colored dot calling `onToggle(key)`. COMMS dot colors match the map marker colors exactly.
...
- **MapView route layer**: `route-source` (GeoJSON, empty at init) + `route-line-outline` (white glow, width 9, opacity 0.25, slot `top`) + `route-line` (width 5, slot `top`) added in `style.load`. `useEffect([plannedRoute, routeProfile])` updates source data and repaints `route-line` color via `setPaintProperty`. `useEffect([routeWaypoints])` creates/destroys numbered `mapboxgl.Marker` elements. Markers cleaned up on unmount. The outline layer is not repainted — it stays white to provide contrast on any basemap tile.
...
- **`POST /api/route-intelligence`**: accepts `{ routeGeometry: GeoJSON.LineString, vehicle: VehicleProfile }`. Runs two PostGIS queries in parallel — roads and bridges. Roads query: fast GiST-indexed `ST_DWithin(r.geom, route, 0.0004°≈22 m)` candidate pass, then a **10 m corridor overlap filter** (`ST_Buffer(route::geography, 10m)` CTE + `ST_Length(ST_Intersection(road, corridor)::geography) > 15`) that rejects side roads touching the route only at a junction (perpendicular overlap ≈ 10 m < threshold). Hazard pin uses `ST_ClosestPoint`. Bridges: `ST_DWithin` 50 m (point geometry, no overlap filter needed). Classifies each row against vehicle profile: 10 road rules (mass/bogie/axle/width/height limits, damage, condition class, rut depth, pavement) and 5 bridge rules (vehicle/bogie/axle mass, height, status). Infantry (`mass_t=0`) skips all mass-based checks. Sorts CRITICAL→WARNING→INFO. Returns `{ hazards[], summary: { critical, warning, info, passable } }`. 503 when `DATABASE_URL` absent, 400 on bad input.
...
- **`CustomLayerSection`** (rendered inside `LayerPanel`): list with toggle checkbox, colour dot, active-drawing selector (click layer name), inline delete confirm, inline create form. The legacy `CustomLayerPanel` default export (standalone positioned panel) is no longer used in `MapWithNav` but is kept for test compatibility.
