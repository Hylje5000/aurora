...
│   │                           #   COMMS section shows one "Cell Towers" row (controls cellGSM/UMTS/LTE/CDMA via onToggleComms prop);
│   │                           #   content div has max-h-[calc(100vh-10rem)] overflow-y-auto for scroll when custom layers grow
...
- **Weather widget**: When `selectedAreaId !== null`, `MapWithNav` renders a weather panel (`absolute left-4 top-10 z-10 w-52`). The panel has a single compact header row (`py-1.5 border-b`) containing the area name (`text-xs font-bold flex-1 truncate`) and `DatePicker bare` side-by-side, then `WeatherWidget bare` below. Both `WeatherWidget` and `DatePicker` accept a `bare?: boolean` prop; when true they omit their outer panel div. `WeatherWidget` fetches `/api/weather?region&month&day` on prop change; shows avg temp ± spread, ↑max/↓min, rain probability %, avg mm on wet days. Falls back to "No data" when `sampleSize === 0`. `DatePicker` renders two compact `<select>` dropdowns (`px-1.5 py-0.5`); Feb max is 29.
...
- **LayerPanel**: Floating dark-slate panel (`absolute left-4 bottom-10 w-56`) with collapsible sections — BASEMAP, TERRAIN, ELEVATION, VEGETATION, COMMS, INFRASTRUCTURE, and optionally CUSTOM LAYERS. Accepts `customLayerProps?: CustomLayerPanelProps` and `onToggleComms: () => void`. Content div has `max-h-[calc(100vh-10rem)] overflow-y-auto` to scroll when custom layers grow. COMMS section shows a single "Cell Towers" row (checked when any of cellGSM/UMTS/LTE/CDMA is on) calling `onToggleComms`, plus a "Coverage Circles" row calling `onToggle`. `handleCommsToggle` in `MapWithNav` sets all four cell keys atomically via one `setLayerVisibility` call.
